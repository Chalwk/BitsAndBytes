...
function love.load()
    love.window.setTitle("BitsAndBytes - Digital Adventure")
...
function love.mousepressed(x, y, button, istouch)
    if gameState == "menu" then
        ...
        if action == "start" then
            gameState = "playing"
            game:startNewGame(menu:getDifficulty(), menu:getGameMode())
    ...
    elseif gameState == "options" then
        ...
        elseif action:sub(1, 8) == "gameMode" then
            local gameMode = action:sub(10)
            menu:setGameMode(gameMode)
        end
    elseif gameState == "playing" then
